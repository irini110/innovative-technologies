...
AFRAME.registerComponent('planet', {
 ...
 AFRAME.registerComponent('main', {
  ...
  tick: function (time, deltaTime) {
    const dt = deltaTime / 1000; // Коррекция крока интегрирования в секунды
    const G = 6.67e-11; // Гравитационная постоянная

    for (var i = 0; i < this.solar_system.length; i++) {
      var planet_i = this.solar_system[i].getAttribute('planet');
      planet_i.a = [0, 0, 0];

      for (var j = 0; j < this.solar_system.length; j++) {
        if (i != j) {
          var planet_j = this.solar_system[j].getAttribute('planet');
          var deltapos = [0, 0, 0];
          ...
          var r = Math.sqrt(
            Math.pow(deltapos[0], 2) + Math.pow(deltapos[1], 2) + Math.pow(deltapos[2], 2)
          );
      ...
      this.solar_system[i].setAttribute('planet', planet_i);
    }
  }
});
